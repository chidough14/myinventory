...
class Manage 
{
  ...
  public function manageRecordWithPagination($table,$pno){
		...
		if ($table == "categories") {
      $sql = "SELECT p.id,p.category_name as category, c.category_name as parent, p.status FROM categories p LEFT JOIN categories c ON p.parent_cat=c.id ".$a["limit"];
		} else if($table == "products"){
			$sql = "SELECT p.pid,p.product_name,c.category_name,b.brand_name,p.product_price,p.product_stock,p.added_date,p.p_status FROM products p,brands b,categories c WHERE p.bid = b.bid AND p.cid = c.id ".$a["limit"];
		} else {
  ...
  //LIMIT 0,10
	...
	public function storeCustomerOrderInvoice ($order_date, $cust_name, $arr_tqty, $arr_qty,
		...
		if($invoice_no != null) {
			for($i = 0; $i < count($arr_price); $i++) {
				...
				$rem_qty = $arr_tqty[$i] - $arr_qty[$i];  //finding remaining quantity
...
//$obj = new Manage();
//echo "<pre>";
//print_r($obj->getSingleRecord("categories", "id", 1));

//echo $obj->deleteRecord("products", "pid", 1);
//echo $obj->update_record("categories",["id"=>1],["parent_cat"=>0,"category_name"=>"Smart Phones","status"=>1]);
